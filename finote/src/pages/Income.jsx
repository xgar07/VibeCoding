...
import { Plus, Edit2, Trash2, TrendingUp, Filter, Search } from 'lucide-react'
...
const Income = () => {
  ...
  const { income, totalIncome, loading, addTransaction, updateTransaction, deleteTransaction, refetch } = useTransactions(selectedMonth)
  ...
  const handleSubmit = async (data) => {
    ...
    if (result.error) { toast.error('Gagal menyimpan: ' + result.error.message) }
    else { toast.success(editData ? 'Diperbarui!' : 'Pemasukan ditambahkan! 💰'); setShowModal(false); setEditData(null) }
  }
  ...
  const handleDelete = async () => {
    ...
    else { toast.success('Dihapus'); setDeleteId(null) }
  }
    ...
    <div className="page-container">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div>
          ...
          <p className="page-subtitle mt-0.5">Kelola semua sumber pemasukanmu</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <ExportImportMenu transactions={income} onImport={handleImport} />
      ...
      {/* Summary */}
      <div className="card p-4 sm:p-5" style={{ background: 'linear-gradient(135deg, rgba(34,197,94,0.08), var(--bg-card))', borderColor: 'rgba(34,197,94,0.2)' }}>
        <p className="text-xs" style={{ color: 'var(--text-muted)' }}>Total Pemasukan Bulan Ini</p>
        <p className="text-2xl sm:text-3xl font-bold text-green-400 mt-1 tabular-nums">{formatCurrency(totalIncome)}</p>
        <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>{income.length} transaksi</p>
      </div>
        ...
        {loading ? (
          ...
          <div style={{ borderRadius: 16 }}>
            {filtered.map((tx, i) => (
              ...
              >
                <CategoryBadge category={tx.category} type="income" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate" style={{ color: 'var(--text-primary)' }}>{tx.description || 'Pemasukan'}</p>
                  <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{formatDate(tx.date)}</p>
                </div>
                <span className="text-sm font-bold text-green-400 tabular-nums whitespace-nowrap">+{formatCurrency(tx.amount)}</span>
          ...
          <EmptyState variant="income" onCta={openAdd} ctaLabel="Tambah Pemasukan" />
        )}
      </div>

      <Modal isOpen={showModal} onClose={() => { setShowModal(false); setEditData(null) }}
        title={editData ? 'Edit Pemasukan' : 'Tambah Pemasukan'}>
        <TransactionForm type="income" initialData={editData} onSubmit={handleSubmit}
          onCancel={() => { setShowModal(false); setEditData(null) }} loading={formLoading} />
      </Modal>

      <ConfirmDialog isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete}
        title="Hapus Pemasukan?" message="Transaksi ini akan dihapus permanen." loading={deleteLoading} />
    </div>
  )
}
